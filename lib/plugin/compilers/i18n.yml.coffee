...
compilers.i18n_yml = (compileStep) ->
  ...
  if _.isUndefined(language) or _.isEmpty(language)
    compileStep.error
      message: "Language-tag is not specified for *.i18n.yml file: `#{input_path}'",
  ...
  translations = helpers.loadYAML input_path, compileStep
  ...
  output_path = compileStep.rootOutputPath + compileStep.inputPath.replace /yml$/, "js"
