 ...
 *
 */
wrench.readdirSyncRecursive = function(baseDir, filter) {
 ...
 *
 */
wrench.readdirRecursive = function(baseDir, fn) {
    baseDir = baseDir.replace(/\/$/, '');
 ...
 */
wrench.rmdirSyncRecursive = function(path, failSilent) {
    var files;
...
wrench.copyDirSyncRecursive = function(sourceDir, newDirLocation, opts) {
    ...
    for(var i = 0; i < files.length; i++) {
        ...
        if(currFile.isDirectory()) {
            ...
            wrench.copyDirSyncRecursive(_path.join(sourceDir, files[i]), _path.join(newDirLocation, files[i]), opts);
        } else if(currFile.isSymbolicLink()) {
            ...
            if (tmpCurrFile.isDirectory()) {
                wrench.copyDirSyncRecursive(symlinkFull, _path.join(newDirLocation, files[i]), opts);
            } else {
...
wrench.chmodSyncRecursive = function(sourceDir, filemode) {
    ...
    for(var i = 0; i < files.length; i++) {
        ...
        if(currFile.isDirectory()) {
            ...
            wrench.chmodSyncRecursive(_path.join(sourceDir, files[i]), filemode);
        } else {
            /*  At this point, we've hit a file actually worth copying... so copy it on over. */
 ...
 *  Note: Directories should be passed to this function without a trailing slash.
 */
wrench.chownSyncRecursive = function(sourceDir, uid, gid) {
    var files = fs.readdirSync(sourceDir);
    ...
    for(var i = 0; i < files.length; i++) {
        ...
        if(currFile.isDirectory()) {
            /*  ...and recursion this thing right on back. */
            wrench.chownSyncRecursive(_path.join(sourceDir, files[i]), uid, gid);
        } else {
 ...
 */
wrench.rmdirRecursive = function rmdirRecursive(dir, failSilent, clbk){
 ...
 *  Note: Directories should be passed to this function without a trailing slash.
 */
wrench.copyDirRecursive = function copyDirRecursive(srcDir, newDir, opts, clbk) {
    ...
    fs.stat(newDir, function(err, newDirStat) {
        if(!err) {
            if(typeof opts !== 'undefined' && typeof opts !== 'function' && opts.forceDelete)
                return wrench.rmdirRecursive(newDir, function(err) {
...
wrench.mkdirSyncRecursive = mkdirSyncRecursive;

wrench.LineReader = function(filename, bufferSize) {
