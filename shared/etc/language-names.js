...
const LANGUAGE_NAMES = {
    ...
    "zh-HK": [ "Chinese (Hong Kong)", "繁体中文（香港）" ],
    "zh-TW": [ "Chinese (Taiwan)", "繁体中文（台湾）" ],
    "co": [ "Corsican", "corsu" ],
...
function tagToLanguage( langTag ){
    let tag = langTag;
    if( typeof LANGUAGE_NAMES[ tag ] === 'undefined' ){
        tag = tag.split( '-' )[0];
    }

    const { [ tag ]: [ en, name ] = [] } = LANGUAGE_NAMES;
    return {
        [ langTag ]: {
            name,
            en
